...
class ExportsIterator(APIResultsIterator):
    ...
    def _get_page(self):
        ...
        def get_status():
            ...
            status = self._api.get('v3/{}/export/{}/status'.format(self.type, self.uuid)).json()
        ...
        while not downloaded and counter <= 3:
            try:
                self.page = self._api.get('v3/{}/export/{}/chunks/{}'.format(
    ...
    def cancel(self):
        ...
        Cancels the export.
        '''
        self._api.get('v3/{}/export/{}/cancel'.format(self.type, self.uuid)).json()
...
class ExportsAPI(TIOEndpoint):
    ...
    def vulns(self, **kw):
        ...
        if not uuid:
            uuid = self._api.post(
                'v3/vulns/export', json=payload).json()['export_uuid']
    ...
    def assets(self, **kw):
        ...
        if not uuid:
            uuid = self._api.post(
                'v3/assets/export', json=payload).json()['export_uuid']
    ...
    def compliance(self, **kw):
        ...
        if not uuid:
            uuid = self._api.post(
                'v3/compliance/export', json=payload).json()['export_uuid']
