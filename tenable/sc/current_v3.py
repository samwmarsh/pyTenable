...
class CurrentSessionAPI(SCEndpoint):
    def associate_cert(self):
        ...
        '''
        return self._api.post('v3/currentUser/associateCert').json()['response']
    ...
    def org(self, fields=None):
        ...
        if fields:
            params['fields'] = ','.join([self._check('field', f, str)
                                         for f in fields])
        return self._api.get('v3/currentOrganization', params=params).json()['response']
    ...
    def user(self, fields=None):
        ...
        if fields:
            params['fields'] = ','.join([self._check('field', f, str)
                                         for f in fields])
        return self._api.get('v3/currentUser', params=params).json()['response']
